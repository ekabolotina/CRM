...
if (
    $car_make &&
    $car_model &&
    ctype_digit($car_make)
) {
    if(
        ($query = $link->query("SELECT id FROM `car_makes` WHERE id = $car_make")) &&
        $query->num_rows)
    {
        if(
            ($query = $link->query("SELECT id FROM `car_models` WHERE name = '$car_model' AND car_make = $car_make")) &&
            !$query->num_rows)
        {
